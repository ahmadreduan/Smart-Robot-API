...
public class OrderService {
    ...
    public Order createOrder(String locationCode) {
        log.info("Creating order for location: {}", locationCode);
        Order order = new Order();
        order.setLocationCode(locationCode);
        order.setPin(String.valueOf(1000 + random.nextInt(9000)));
        order.setStatus("PENDING");
        return orderRepository.save(order);
    }

    public Optional<Order> getOrderByLocationCode(String locationCode) {
        log.info("Fetching order for location: {}", locationCode);
        return orderRepository.findByLocationCode(locationCode);
    }
